...
class WorkerRobot(Robot):
    def __init__(self, name, model, power_level, tool, file):
    ...
    def __enter__(self):
        ...
        self.file.write(self.power_level)
        return self.file
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if self.file:
            self.file.close()
        if exc_type:
            print(f'Something went wrong! {self.exc_type}')

    def __del__(self):
        print('The object is cleared')
...
objBob = WorkerRobot('Bob', 'X1', 100, 'Wrench', 'Text.txt')
